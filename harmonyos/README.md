...
- 网络：搜索、安装插件、拉取音源、下载、WebDAV
- 后台运行：熄屏后继续播放
- 读取音频：导入本地音乐
- 悬浮窗：桌面歌词（普通应用通常申请不到，会回退为应用内歌词条）

## 已知限制

相对原版 Android 仍有差距：

- 真·桌面 overlay 歌词受系统权限限制，第三方应用多半只能用应用内歌词条
- 插件沙箱已补 AES/SHA/HMAC、cheerio、axios 常用子集，极端依赖完整 Node 生态的插件仍可能失败
- 权限页、文件选择器、i18n 多语言尚未对齐；鸿蒙版检查更新只提示，不提供 HAP 商店安装
- 需要在 DevEco Studio 中编译，本仓库不包含 HAP 安装包

## 协议

与原项目相同，遵循 **AGPL 3.0**。二次分发请保留出处：https://github.com/maotoumao/MusicFree
